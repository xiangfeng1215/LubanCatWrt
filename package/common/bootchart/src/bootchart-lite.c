...
#define BUFSIZE        2048
#define EXIT_PROC      "bootchart-end.sh"
#define EXIT_COUNT_SECOND 200

static unsigned int count_second = 0;
...
int main(int argc, char **argv)
{
	...
	/* create header file */
	if ((header_fd=open(LOGTMP "/header", O_WRONLY | O_TRUNC | O_CREAT, 0755))==-1) {
	...
	while(running) {
		...
		usleep(100000);
		count_second++;
		if(count_second >= EXIT_COUNT_SECOND){
			count_second = 0;
			break;
		}
	}
